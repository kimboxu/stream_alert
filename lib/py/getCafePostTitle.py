...
class getCafePostTitle:
    ...
    async def getCafeDataDic(self):
        try:
            response = await get_message(
                self.performance_manager, "cafe", self.BASE_URL
            )  # API 요청

            # API 응답에서 게시글 목록 추출
            article_list = (
                response.get("message", {}).get("result", {}).get("articleList", [])
            )
            ...
            for article in reversed(article_list):
                ...
                if (
                    ...
                    not in self.cafeData.loc[self.channel_id, "cafeNameDict"]
                ):
                    continue

                # 새 게시글인지 확인 (ID가 더 크고, 작성 시간이 더 최신)
                if not (
                    article["refArticleId"] > max(self.cafe_json["refArticleId"] + [0])
                    and article["writeDateTimestamp"]
            ...
            if should_update:
                # 카페 데이터 저장
                asyncio.create_task(
                    saveCafeData(self.init.supabase, self.cafeData, self.channel_id)
                )
    ...
    async def postCafe(self):
        try:
            ...
            print(
                f"{datetime.now()} {post_data.writer_nickname} post cafe {post_data.subject}"
            )
    ...
    async def create_cafe_json(self, post_data: CafePostData) -> dict:
        ...
        # 카페 게시판 URL 구성
        menu_url = (
            f"https://cafe.naver.com/{cafe_info['cafeID']}"
        ...
        return {
            ...
            "avatar_url": await self.get_cafe_thumbnail_url(
                post_data.writer_nickname
            ),  # 프로필 이미지
            "embeds": [embed],  # 임베드 배열
        }
    ...
    async def get_cafe_thumbnail_url(self, writerNickname: str) -> str:
        ...
        try:
            ...
            # 카페 데이터에서 작성자 정보 추출
            cafe_info = self.cafeData.loc[self.channel_id, "cafeNameDict"][
                writerNickname
            ]
            platform, user_id, current_thumbnail = (
                cafe_info[1],
                cafe_info[0],
                cafe_info[2],
            )
            ...
            data = await get_message(
                self.performance_manager, platform, config["get_link"](user_id)
            )
            ...
            _, _, profile_image = config["process_data"](
                data, user_id, current_thumbnail
            )

            # 새 썸네일 URL 저장
            self.cafeData.loc[self.channel_id, "cafeNameDict"][writerNickname][
                2
            ] = profile_image
