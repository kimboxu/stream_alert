...
def init_background_tasks():
    ...
    init = state.get_init()
    if init is None:
        init = loop.run_until_complete(state.initialize())
    loop.close()
    return init
...
def login():
    ...
    # 로그인 정보 출력 (디버깅용)
    print(
        f"{datetime.now()} 로그인 시도: 사용자명: {username}, 디스코드 웹훅 URL: {discordWebhooksURL}"
    )
...
def register():
    ...
    # 로그인 정보 출력 (디버깅용)
    print(
        f"{datetime.now()} 회원가입 시도: 사용자명: {username}, 디스코드 웹훅 URL: {discordWebhooksURL}"
    )
    print(check_have_id)
    # 인증 로직
    if check_have_id == "OK":
...
def save_user_settings():
    ...
    for field in text_fields + json_fields:
        if field in data:
            ...
            if field in json_fields:
                ...
                app.init.userStateData.at[discordWebhooksURL, field] = update_data[
                    field
                ]
    ...
    try:
        ...
        return (
            jsonify(
                {"status": "error", "message": f"데이터베이스 저장 오류: {str(e)}"}
            ),
            500,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "status": "error",
                    "message": f"사용자 이름 변경 중 오류 발생: {str(e)}",
                }
            ),
            500,
        )
...
def get_streamers():
    try:
        ...
        def convert_platform_data(data_dict):
            return {
                platform: df.to_dict("records") for platform, df in data_dict.items()
            }

        response_data = {
            "status": "success",
            "streamers": convert_platform_data(app.init.IDList),
            "cafeStreamers": app.init.cafeData.to_dict("records"),
            "videoDataStreamers": convert_platform_data(app.init.video_data),
            "youtubeStreamers": app.init.youtubeData.to_dict("records"),
            "chatFilter": convert_platform_data(app.init.chatFilter),
        }
...
def proxy_image():
    ...
    try:
        ...
        return (
            jsonify(
                {"status": "error", "message": f"이미지 프록시 처리 중 오류: {str(e)}"}
            ),
            500,
        )
...
def mark_notifications_read():
    ...
    try:
        ...
        success = loop.run_until_complete(
            file_notification_manager.save_notifications(
                discordWebhooksURL,
                updated_notifications,
        ...
        if success:
            return jsonify(
                {"status": "success", "message": "알림이 읽음으로 표시되었습니다"}
            )
        else:
            return (
                jsonify(
                    {"status": "error", "message": "알림 상태 업데이트에 실패했습니다"}
                ),
                500,
            )
...
def clear_notifications():
    ...
    try:
        ...
        success = loop.run_until_complete(
            file_notification_manager.save_notifications(
                discordWebhooksURL, [], force_save=True  # 빈 목록
            )
        )

        if success:
            return jsonify(
                {"status": "success", "message": "모든 알림이 삭제되었습니다"}
            )
        else:
            return (
                jsonify({"status": "error", "message": "알림 삭제에 실패했습니다"}),
                500,
            )
...
def register_fcm_token():
    ...
    try:
        ...
        for other_webhook_url in app.init.userStateData.index:
            ...
            has_this_device = any(
                token_item.get("device_id") == device_id
                for token_item in other_tokens_data
            )
            ...
            if has_this_device:
                ...
                if len(updated_tokens) != len(other_tokens_data):
                    ...
                    print(
                        f"{datetime.now()} 다른 사용자({other_webhook_url[:10]}...)에서 기기 ID({device_id})를 가진 토큰을 제거했습니다."
                    )
...
def remove_fcm_token():
    ...
    try:
        ...
        if device_id:
            # 기기 ID로 제거
            tokens_data = [
                item for item in tokens_data if item.get("device_id") != device_id
            ]
        elif fcm_token:
            # 특정 토큰 제거
            tokens_data = [
                item for item in tokens_data if item.get("token") != fcm_token
            ]
        else:
...
def get_active_highlight_chats():
    ...
    try:
        ...
        # 특정 채널 또는 전체 채널 처리
        channels_to_check = (
            [channel_id] if channel_id else list(app.init.highlight_chat.keys())
        )
            ...
            for stream_id, highlight_data in channel_data.items():
                try:
                    ...
                    basic_info = {
                        ...
                        "timeline_comments_count": len(
                            getattr(highlight_data, "timeline_comments", [])
                        ),
                        "has_ended": bool(getattr(highlight_data, "stream_end_id", "")),
                        "created_time": (
                            get_timestamp_from_stream_id(stream_id).isoformat()
                            if stream_id
                            else None
                        ),
                    }
                        ...
                        basic_info.update(
                            {
                                ...
                                "latest_comments": (
                                    timeline_comments[-5:] if timeline_comments else []
                                ),  # 최근 5개
                            }
                        )
                    ...
                    print(
                        f"{datetime.now()} 하이라이트 데이터 처리 오류 ({ch_id}, {stream_id}): {str(e)}"
                    )
                    continue
            ...
            try:
                ...
                for platform in platform_list:
                    if ch_id in app.init.IDList[platform].index:
                        channel_name = app.init.IDList[platform].loc[
                            ch_id, "channelName"
                        ]
        ...
        return jsonify(
            {
                ...
                "total_active_streams": sum(
                    data["active_streams"] for data in result.values()
                ),
                "data": result,
...
def get_highlight_chat_detail():
    ...
    try:
        ...
        if channel_id and stream_id:
            if (
                channel_id in app.init.highlight_chat
                and stream_id in app.init.highlight_chat[channel_id]
            ):
        ...
        # 파일에서 조회 (저장된 데이터)
            ...
            try:
                ...
                return (
                    jsonify(
                        {"status": "error", "message": f"파일 읽기 실패: {str(e)}"}
                    ),
                    500,
                )
        ...
        if result_data and "timeline_comments" in result_data:
            ...
            stats = {
                ...
                "comments_with_scores": len(
                    [c for c in comments if "score_difference" in c]
                ),
                "avg_score": 0,
                "max_score": 0,
                "score_distribution": {
...
def get_highlight_chat_statistics():
    ...
    try:
        include_channel_breakdown = (
            request.args.get("include_channels", "false").lower() == "true"
        )
            ...
            for stream_id, highlight_data in channel_data.items():
                ...
                try:
                    ...
                    try:
                        ...
                        if (
                            datetime.now() - stream_time
                        ).total_seconds() < 3600:  # 1시간
                    ...
                    print(
                        f"{datetime.now()} 스트림 통계 처리 오류 ({channel_id}, {stream_id}): {str(e)}"
                    )
                    continue
        ...
        try:
            ...
            saved_stats = {
                ...
                "total_saved_highlights": sum(
                    f.get("total_highlights", 0) for f in saved_files
                ),
                "storage_size_mb": sum(f.get("size_mb", 0) for f in saved_files),
                "date_range": (
                    {
                        "oldest": min(
                            (f.get("stream_start_time", "") for f in saved_files),
                            default="",
                        ),
                        "newest": max(
                            (f.get("stream_start_time", "") for f in saved_files),
        ...
        return jsonify(
            {
                ...
                "system_health": {
                    "memory_usage_normal": active_stats["total_active_streams"]
                    < 100,  # 임계치 기준
                    "recent_activity_detected": len(active_stats["recent_activity"])
                    > 0,
                    "storage_healthy": saved_stats["storage_size_mb"]
                    < 1000,  # 1GB 기준
                },
                "retrieved_at": datetime.now().isoformat(),
            }
...
def cleanup_highlight_data():
    ...
    try:
            ...
            try:
                ...
                for stream_id in list(channel_data.keys()):
                    try:
                        ...
                        # 정리 조건 확인
                        should_cleanup = stream_timestamp < threshold_time or (
                            force_cleanup
                            and bool(
                                getattr(channel_data[stream_id], "stream_end_id", "")
                            )
                        )
                        ...
                        if should_cleanup:
                            # 댓글 수 계산
                            comments = getattr(
                                channel_data[stream_id], "timeline_comments", []
                            )
        ...
        return (
            jsonify(
                {"status": "error", "message": f"하이라이트 데이터 정리 실패: {str(e)}"}
            ),
            500,
        )
...
def get_memory_highlights_summary():
    ...
    try:
        ...
        memory_summary = {
            ...
            "total_unsaved_highlights": sum(
                info["highlights_count"] for info in instances_with_highlights
            ),
    ...
    try:
        ...
        def generate_sse_updates():
            ...
            try:
                ...
                thread.start()
                print(
                    f"{datetime.now()} [{task_id}] 하이라이트 저장 작업 시작됨 (백그라운드 스레드에서 실행 중)"
                )
                ...
                while True:
                    ...
                    if current_progress != last_progress:
                        ...
                        print(
                            f"{datetime.now()} [{task_id}] 진행율: {current_progress}% - {task_data.get('message', '')}"
                        )
                    ...
                    if status in ["completed", "error"]:
                        ...
                        yield f"data: {dumps(task_data, ensure_ascii=False)}\n\n"
                        print(
                            f"{datetime.now()} [{task_id}] 작업 {status}: {task_data.get('message', '')}"
                        )
                        break
        ...
        return jsonify({"status": "error", "message": f"SSE 연결 실패: {str(e)}"}), 500
...
async def background_save_task_with_state_manager(task_id):
    ...
    try:
            ...
            async with semaphore:
                try:
                    ...
                    while elapsed_time < max_wait_time:
                        # 현재 상태 확인
                        is_completed = init.wait_make_highlight_chat.get(
                            channel_id, True
                        )
                        ...
                        if elapsed_time % 10 == 0:
                            print(
                                f"{datetime.now()} [{platform}] {channel_name}: 작업 대기 중... ({elapsed_time}초)"
                            )

                    if elapsed_time >= max_wait_time:
                        print(
                            f"{datetime.now()} [{platform}] {channel_name}: 작업 시간 초과 (최대 {max_wait_time}초)"
                        )
                    ...
                    result = {
                        ...
                        "wait_completed": init.wait_make_highlight_chat.get(
                            channel_id, True
                        ),
        ...
        tasks = [
            process_single_channel(instance) for instance in instances_with_highlights
        ]
        ...
        print(
            f"{datetime.now()} 하이라이트 저장 작업 완료: 총 {save_results['total_highlights_saved']}개"
        )
...
def get_task_status(task_id):
    try:
        ...
        if not stream:
            ...
            return jsonify({"status": "success", "task_data": task_data})
            ...
            try:
                ...
                while True:
                    ...
                    if (
                        ...
                        print(
                            f"{datetime.now()} [Task {task_id[:8]}...] 상태 업데이트: {current_status} - {current_progress}%"
                        )
        ...
        return (
            jsonify({"status": "error", "message": f"작업 상태 조회 실패: {str(e)}"}),
            500,
        )
...
def cleanup_old_tasks():
    ...
    try:
        ...
        return jsonify({"status": "error", "message": f"작업 정리 실패: {str(e)}"}), 500
...
def get_performance_stats():
    try:
        ...
        api_type = request.args.get(
            "api_type", default=None, type=str
        )  # 특정 API 타입 필터
        ...
        if not performance_manager:
            return (
                jsonify(
                    {"status": "error", "message": "성능 매니저를 사용할 수 없습니다"}
                ),
                500,
            )
        ...
        try:
            ...
            stats = loop.run_until_complete(
                performance_manager.get_statistics(
                    days=days, stat_type="realtime", api_type=api_type
                )
            )
            ...
            if stats:
                ...
                return jsonify({"status": "error", "message": "통계 계산 실패"}), 500

        finally:
            loop.close()

    except Exception as e:
        return jsonify({"status": "error", "message": f"통계 조회 실패: {str(e)}"}), 500
    ...
    try:
        ...
        try:
            health_stats = loop.run_until_complete(
                performance_manager.get_statistics(
                    days=days, stat_type="api_health", api_type=api_type
                )
            )

            return jsonify({"status": "success", **health_stats})
        ...
        return (
            jsonify({"status": "error", "message": f"API 건강도 조회 실패: {str(e)}"}),
            500,
        )
    ...
    try:
        ...
        try:
            summary = loop.run_until_complete(
                performance_manager.get_statistics(
                    days=days, stat_type="performance_summary"
                )
            )
        ...
        return (
            jsonify({"status": "error", "message": f"성능 요약 조회 실패: {str(e)}"}),
            500,
        )
...
def compare_apis():
    try:
        ...
        return (
            jsonify({"status": "error", "message": f"API 비교 분석 실패: {str(e)}"}),
            500,
        )
...
def get_daily_statistics():
    try:
        days = request.args.get("days", default=7, type=int)
        summary = (
            request.args.get("summary", default="false", type=str).lower() == "true"
        )
        api_type = request.args.get("api_type", default=None, type=str)
        ...
        try:
            if summary:
                ...
                result = loop.run_until_complete(
                    performance_manager.get_statistics(
                        days=days, stat_type="daily_summary"
                    )
                )
            else:
                # 기본 일일 통계 리스트
                result = loop.run_until_complete(
                    performance_manager.get_statistics(
                        days=days, stat_type="daily", api_type=api_type
                    )
                )
        ...
        return (
            jsonify({"status": "error", "message": f"일일 통계 조회 실패: {str(e)}"}),
            500,
        )
...
def analyze_api():
    try:
        ...
        return jsonify({"status": "error", "message": f"API 분석 실패: {str(e)}"}), 500
...
def get_active_api_types():
    try:
        ...
        return (
            jsonify(
                {"status": "error", "message": f"활성 API 타입 조회 실패: {str(e)}"}
            ),
            500,
        )
...
def set_performance_alerts():
    try:
        ...
        return (
            jsonify({"status": "error", "message": f"성능 알림 설정 실패: {str(e)}"}),
            500,
        )
    ...
    try:
        ...
        try:
            ...
            if result:
                ...
                return (
                    jsonify(
                        {"status": "error", "message": "일일 통계 계산에 실패했습니다"}
                    ),
                    500,
                )
        ...
        return (
            jsonify({"status": "error", "message": f"일일 통계 계산 실패: {str(e)}"}),
            500,
        )
...
if __name__ == "__main__":
    ...
    if environ.get("WERKZEUG_RUN_MAIN") != "true":
        ...
        if not firebase_initialized_globally:
            print(
                f"{datetime.now()} 경고: Firebase 초기화에 실패했습니다. 푸시 알림 기능이 작동하지 않을 수 있습니다."
            )
