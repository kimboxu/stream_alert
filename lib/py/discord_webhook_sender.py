...
# 디스코드 웹훅과 함께 푸시 알림도 보내기
def get_list_of_urls_and_tokens(DO_TEST, userStateData, name, channel_id, json_data, db_name):
    result_urls = []
    fcm_tokens_list = []
    ...
    try:
        if DO_TEST:
            for _ in range(1):
                ...
                discordWebhookURL = "https://discord.com/api/webhooks/1263062609792602113/q6IL7qk-o6PlYhnAk1cuqX6aphAW5AHqrHtO3xCozZXyGNxoUcdgP2uoV4davyUDp3ql"
                fcm_tokens = userStateData.loc[discordWebhookURL, "fcm_tokens"]
                if fcm_tokens and isinstance(fcm_tokens, str):
                    fcm_tokens_list.extend([token.strip() for token in fcm_tokens.split(',') if token.strip()])
            return result_urls, fcm_tokens_list
        ...
        asyncio.create_task(DiscordWebhookSender._log_error(f"Error in get_list_of_urls: {type(e).__name__}: {str(e)}"))
        return result_urls, []
