...
def normalize_discord_webhook_url(webhook_url: str) -> str:
    if webhook_url is None:
        return None
    return webhook_url.replace("https://discordapp.com/api/webhooks/", "https://discord.com/api/webhooks/")
...
def get_notifications():
    ...
    limit = request.args.get('limit', default=50, type=int)  # 한 번에 가져올 알림 수 제한
...
def remove_fcm_token():
    ...
    try:
        ...
        return jsonify({"status": "error", "message": f"토큰 제거 중 오류 발생: {str(e)}"}), 500
    ...
    try:
        ...
        with app.app_context():
            ...
            for discord_webhook_url in messages:
                ...
                asyncio.create_task(
                    post_msg_to_flutter(user_data, json_data)
                )
