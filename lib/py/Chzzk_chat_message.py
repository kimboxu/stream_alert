    ...
    async def _message_receiver(self, message_queue: asyncio.Queue):
        async def should_close_connection():
            ...
            if  is_change_chatChannel or is_new_chatChannel or check_chat or is_old_chatChannel:
                asyncio.create_task(log_error(f"test should_close_connection {self.data.channel_name} is_change_chatChannel:{is_change_chatChannel},is_new_chatChannel:{is_new_chatChannel}, check_chat:{check_chat},is_old_chatChannel:{is_old_chatChannel}"))
