...
class afreeca_chat_message(ChatMessageWithAnalyzer):
    ...
    def get_user_chat(self, messages):
        ...
        if len(messages) == 14:
            ...
            user_id, chat, nickname, chat_type = messages[6], messages[2], messages[7], "이미지 챗"
        else:
    ...
    async def check_is_passwordDict(self):
        stateData = await get_message(self.performance_manager, "afreeca", afreeca_getLink(self.init.IDList["afreeca"]["uid"][self.data.channel_id]))
        return (
