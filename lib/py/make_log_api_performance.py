...
class APIPerformanceLogger:
    ...
    def __init__(self, log_dir: Path = None, max_memory_logs: int = 10000):
        ...
        self._thread_pool = ThreadPoolExecutor(
            max_workers=2, thread_name_prefix="perf_logger"
        )
    ...
    def force_save_sync(self):
        ...
        try:
            ...
            if logs_to_save:
                ...
                self._last_save_time = datetime.now()
                print(
                    f"{datetime.now()} 스케줄러: 강제 저장 완료 ({len(logs_to_save)}개)"
                )
    ...
    async def _check_and_save_if_needed(self):
        ...
        if (
            time_diff >= self.save_interval_minutes
            or len(self.memory_logs) >= self.max_memory_logs
        ):
        ...
        async with self._save_lock:
            ...
            try:
                ...
                await asyncio.get_event_loop().run_in_executor(
                    self._thread_pool, self._sync_save_logs, logs_to_save
                )
    ...
    def _sync_save_logs(self, logs_to_save):
        ...
        print(
            f"{datetime.now()} API 성능 로그 저장 완료: {file_path} ({len(logs_data)}개 기록)"
        )
    ...
    def _filter_files_by_date_range(
        self, file_paths: List[str], start_date: datetime, end_date: datetime
    ) -> List[str]:
        ...
        for i, (file_path, file_time) in enumerate(files_with_time):
            ...
            prev_time = (
                files_with_time[i - 1][1]
                if i > 0 and files_with_time[i - 1][1] is not None
                else datetime.min
            )
    ...
    async def _read_logs_from_files_async(
        self, start_date: datetime, end_date: datetime
    ) -> List[Dict]:
        """파일에서 로그를 비동기적으로 읽기"""
        ...
        file_paths = self._filter_files_by_date_range(
            all_file_paths, start_date, end_date
        )

        print(
            f"{datetime.now()} 파일 로그 읽기 시작: 전체 {len(all_file_paths)}개 중 {len(file_paths)}개 파일 선택"
        )
        ...
        print(
            f"{datetime.now()} 파일 로그 읽기 완료: {elapsed:.2f}초, {len(all_logs)}개 로그"
        )
    ...
    def get_current_memory_stats(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "next_auto_save_in_minutes": max(
                    0,
                    self.save_interval_minutes
...
class APIStatisticsCalculator:
    ...
    async def calculate_avg_response_time(self, api_type: str, logs) -> float:
        ...
        try:
            filtered_logs = [
                log for log in logs if log["api_type"] == api_type and log["is_success"]
            ]
    ...
    async def calculate_comprehensive_statistics(
        ...
        try:
            ...
            for api_type, acc in stats_acc.items():
                ...
                success_rate = round(
                    (acc["successful_requests"] / acc["total_requests"]) * 100, 2
                )
            ...
            # 전체 요약 통계
            total_requests = sum(
                stats["total_requests"] for stats in api_performance.values()
            )
            total_successful = sum(
                stats["successful_requests"] for stats in api_performance.values()
            )
            total_errors = sum(
                stats["failed_requests"] for stats in api_performance.values()
            )
    ...
    def _calculate_overall_success_rate(
        self, total_successful: int, total_requests: int
    ) -> float:
        """전체 성공률 계산"""
        if total_requests == 0:
            return 100.0
    ...
    async def get_api_performance_summary(self, logs) -> Dict:
        ...
        try:
            ...
            for api_type in api_types:
                ...
                performance_summary.append({"api_type": api_type, **stats})
...
class PerformanceManager:
    ...
    async def get_statistics(
        self, days: int = 7, stat_type: str = "realtime", api_type: str = None
    ) -> Dict:
        """통계 조회"""
    ...
    async def _get_realtime_statistics(
        self, days: int = 7, api_type: str = None
    ) -> Dict:
        ...
        try:
            # 통계 계산
            stats = await self.calculator.calculate_comprehensive_statistics(
                start_date, end_date
            )
    ...
    async def _get_api_health_statistics(
        self, days: int = 7, api_type: str = None
    ) -> Dict:
        """API 건강도 통계"""
        end_date = datetime.now()
        ...
        try:
            ...
            if api_type:
                # 특정 API 타입의 건강도
                health_data = await self.calculator.calculate_api_health_score(
                    api_type, logs
                )
                api_stats = await self.calculator.get_api_statistics_by_type(
                    api_type, logs
                )
                ...
                for api in api_types:
                    health_results[api] = (
                        await self.calculator.calculate_api_health_score(api, logs)
                    )
    ...
    async def _get_performance_summary(self, days: int = 7) -> Dict:
        ...
        try:
            logs = await self.logger.get_logs_in_period(start_date, end_date)
            performance_summary = await self.calculator.get_api_performance_summary(
                logs
            )
    ...
    async def _get_daily_statistics_list(
        self, days: int = 7, api_type: str = None
    ) -> Dict:
        ...
        try:
            ...
            for api_type in all_api_types:
                ...
                api_summaries[api_type] = {
                    "total_requests": total_requests,
                    "daily_avg_requests": (
                        round(total_requests / len(daily_stats_list), 1)
                        if daily_stats_list
                        else 0
                    ),
                    "total_errors": total_errors,
                    "daily_avg_errors": (
                        round(total_errors / len(daily_stats_list), 1)
                        if daily_stats_list
                        else 0
                    ),
                    "avg_response_time_ms": (
                        round(sum(valid_response_times) / len(valid_response_times), 2)
                        if valid_response_times
                        else 0
                    ),
                    "avg_success_rate_percent": (
                        round(sum(valid_success_rates) / len(valid_success_rates), 2)
            ...
            total_all_requests = sum(
                summary["total_requests"] for summary in api_summaries.values()
            )
            total_all_errors = sum(
                summary["total_errors"] for summary in api_summaries.values()
            )
            ...
            return {
                ...
                "overall_summary": {
                    ...
                    "daily_avg_requests": (
                        round(total_all_requests / len(daily_stats_list), 1)
                        if daily_stats_list
                        else 0
                    ),
                    "total_errors": total_all_errors,
                    "daily_avg_errors": (
                        round(total_all_errors / len(daily_stats_list), 1)
                        if daily_stats_list
                        else 0
                    ),
    ...
    async def get_api_comparison(self, api_types: List[str], days: int = 7) -> Dict:
        ...
        try:
            ...
            for api_type in api_types:
                ...
                health = await self.calculator.calculate_api_health_score(
                    api_type, logs
                )
    ...
    async def calculate_and_save_daily_statistics(self, target_date: datetime = None):
        ...
        try:
            ...
            comprehensive_stats = (
                await self.calculator.calculate_comprehensive_statistics(
            ...
            print(
                f"{datetime.now()} 일일 통계 저장 완료: {target_date} (총 {total_time:.2f}초)"
            )
            ...
            print(
                f"총 {summary.get('unique_api_types', 0)}개 API 타입, "
            ...
            for api_type, stats in sorted_apis[:5]:  # 상위 5개만
                print(
                    f"  {api_type}: {stats.get('total_requests', 0)}건, "
    ...
    async def _save_daily_statistics(self, daily_stat):
        ...
        try:
            # 파일 저장을 스레드 풀에서 실행 (논블로킹)
            await asyncio.get_event_loop().run_in_executor(
                self.logger._thread_pool, self._sync_save_daily_statistics, daily_stat
            )
    ...
    def _sync_save_daily_statistics(self, daily_stat):
        ...
        filtered_stats = {
            date: stat
            for date, stat in existing_stats.items()
    ...
    def calculate_and_save_daily_statistics_sync(self, target_date: datetime = None):
        ...
        try:
            ...
            try:
                ...
                elapsed = time.time() - start_time
                print(
                    f"{datetime.now()} 스케줄러: 일일 통계 저장 완료 - {target_date} ({elapsed:.2f}초)"
                )
    ...
    def setup_scheduler(self):
        ...
        if self.scheduler is None:
            ...
            # 프로그램 종료시 스케줄러 정리
            atexit.register(
                lambda: self.scheduler.shutdown() if self.scheduler else None
            )
