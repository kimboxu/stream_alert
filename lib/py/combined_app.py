...
def get_or_create_instance(
    instance_type, init, performance_manager, channel_id, **kwargs
):
    ...
    if instance_type == "cafe":
        ...
        if developer_key:
            new_instance = getYoutubeJsonData(
                init, performance_manager, developer_key, channel_id
            )
    ...
    while True:
        try:
            ...
            cafe_tasks = [
                asyncio.create_task(
                    get_or_create_instance(
                        "cafe", init, performance_manager, channel_id
                    ).start()
                )
            ...
            chzzk_video_tasks = [
                asyncio.create_task(
                    get_or_create_instance(
                        "chzzk_video", init, performance_manager, channel_id
                    ).start()
                )
            ...
            afreeca_video_tasks = [
                asyncio.create_task(
                    get_or_create_instance(
                        "afreeca_video", init, performance_manager, channel_id
                    ).start()
                )
            ...
            chzzk_live_tasks = [
                asyncio.create_task(
                    get_or_create_instance(
                        "chzzk_live", init, performance_manager, channel_id
                    ).start()
                )
                for channel_id in list(init.IDList["chzzk"].index)
            ]
            ...
            afreeca_live_tasks = [
                asyncio.create_task(
                    get_or_create_instance(
                        "afreeca_live", init, performance_manager, channel_id
                    ).start()
                )
                for channel_id in list(init.IDList["afreeca"].index)
            ]
    ...
    while True:
        try:
            ...
            for youtubeChannelID in YoutubeChannelID_list:
                ...
                # 현재 사용할 API 키 결정
                key_index = init.youtube_key_index // len(
                    init.youtubeData["YoutubeChannelID"]
                )
                ...
                # 기존 인스턴스가 있는지 확인
                youtube_instance = state_manager.get_instance_by_type(
                    "youtube", instance_key
                )
                ...
                if youtube_instance is None:
                    ...
                    youtube_instance = getYoutubeJsonData(
                        init, performance_manager, developerKey, youtubeChannelID
                    )
                    state_manager.set_instance(
                        "youtube", instance_key, youtube_instance
                    )
                ...
                # 다음 키로 순환
                init.youtube_key_index = (init.youtube_key_index + 1) % (
                    len(developer_keys) * len(init.youtubeData["YoutubeChannelID"])
                )
...
async def generic_chat(
    init: initVar, performance_manager: PerformanceManager, platform: str
):
    await asyncio.sleep(3)
        ...
        try:
            ...
            for channel_id in list(init.IDList[platform].index):
                if channel_id not in tasks or tasks[channel_id].done():
                    ...
                    chat_instance = get_or_create_instance(
                        chat_class, init, performance_manager, channel_id
                    )
...
async def generic_hot_clip(
    ...
    while True:
        try:
            ...
            for channel_id in list(init.IDList[platform].index):
                if channel_id not in tasks or tasks[channel_id].done():
                    hot_clip_instance = get_or_create_instance(
                        hot_clip_class, init, performance_manager, channel_id
                    )
                    tasks[channel_id] = asyncio.create_task(
                        hot_clip_instance.start_monitoring()
                    )
        ...
        while True:
            try:
                ...
                if notification_queue:
                    ...
                    if queue_size > 100:
                        print(
                            f"{datetime.now()} ⚠️ 알림 큐 크기: {queue_size} (적체 가능)"
                        )
                threading.Event().wait(300)  # 5분마다 확인
            except Exception as e:
                print(f"워커 모니터링 오류: {str(e)}")
