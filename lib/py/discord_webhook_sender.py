...
class DiscordWebhookSender:
    ...
    async def send_messages(
        self, messages: List[str], json_data, DO_TEST=False
    ) -> List[str]:
            ...
            for attempt in range(self.MAX_RETRIES):
                ...
                try:
                    ...
                    async with session.post(
                        ...
                        # 성능 로깅
                        asyncio.create_task(
                            self.performance_manager.log_api_performance(
                    ...
                    print(
                        f"{datetime.now()} Unexpected error sending message: {str(e)}"
                    )
    ...
    async def _log_error(
        self, message: str, is_Do_test, webhook_url=environ.get("errorPostBotURL")
    ):
        try:
            ...
            async with ClientSession() as session:
                ...
                if webhook_url in [environ["chat_post_url"]] and is_Do_test:
                    return
            ...
            print(
                f"{datetime.now()} Failed to log error to webhook: message:{str(message)},e:{str(e)}"
            )
...
def get_list_of_urls(DO_TEST, userStateData, name, channel_id, db_name):
    ...
    try:
        ...
        # 전체 처리 중 오류 발생 시 로깅
        asyncio.create_task(
            DiscordWebhookSender._log_error(
                f"Error in get_cafe_list_of_urls: {type(e).__name__}: {str(e)}"
            )
        )
        return result_urls
