...
class HighlightChatSaver:
    ...
    def _merge_timeline_comments(
        self, existing_comments: List[Dict], new_comments: List[Dict]
    ) -> List[Dict]:
    ...
    async def save_completed_stream_highlight(
        self, channel_id: str, channel_name: str, stream_id: str, highlight_data
    ) -> str:
                ...
                save_data.update(
                    {
                        ...
                        "stream_end_id": new_stream_data.get(
                            "stream_end_id", existing_data.get("stream_end_id", "")
                        ),
                        "stream_end_time": (
                            self._get_stream_end_time(highlight_data)
                            if new_stream_data.get("stream_end_id")
                            else existing_data.get("stream_end_time", "")
                        ),
                        "last_title": new_stream_data.get(
                            "last_title", existing_data.get("last_title", "")
                        ),
                    }
                )
    ...
    def _get_stream_end_time(self, highlight_data) -> str:
        ...
        try:
            if (
                hasattr(highlight_data, "stream_end_id")
                and highlight_data.stream_end_id
            ):
                ...
                if highlight_data["stream_end_id"]:
                    return self._extract_readable_time_from_id(
                        highlight_data["stream_end_id"]
                    )
        except Exception as e:
        ...
        try:
            ...
            for file_path in sorted(
                ...
                try:
                        ...
                        file_info.update(
                            {
                                ...
                                "total_highlights": data.get("statistics", {}).get(
                                    "total_highlights", 0
                                ),
                                "avg_score": data.get("statistics", {}).get(
                                    "avg_score", 0.0
                                ),
                    ...
                    print(
                        f"{datetime.now()} 파일 정보 읽기 실패 ({file_path}): {str(e)}"
                    )
                    continue
