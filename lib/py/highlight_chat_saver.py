...
class HighlightChatSaver:
    ...
    async def save_completed_stream_highlight(self, channel_id: str, channel_name: str, 
        ...
        try:
            ...
            stream_data = self._convert_highlight_data_to_dict(highlight_data)
            print(f"{datetime.now()} [데이터 변환] timeline_comments: {len(stream_data.get('timeline_comments', []))}개")
            ...
            save_data = {
                ...
                "statistics": self._calculate_statistics(stream_data)
            }
            
            print(f"{datetime.now()} [데이터 준비] 완료")
            print(f"  - 타임라인 댓글: {len(save_data['timeline_comments'])}개")
            ...
            print(f"{datetime.now()} 하이라이트 채팅 저장 완료:")
            print(f"  - 파일: {filename}")
            print(f"  - 채널: {channel_name} ({channel_id})")
            print(f"  - 방송 시간: {readable_time}")
            print(f"  - 마지막 방제: {save_data['last_title']}")
